...
import lookids.mono.common.jwt.JwtTokenProvider;
...
public class SignInServiceImpl implements SignInService {
	...
	public SignInResponseDto signIn(SignInRequestDto signInRequestDto) {
		...
		redisTemplate.opsForValue()
			.set(refreshRedisToken.getRefreshToken(), auth.getUuid(), Duration.ofDays(1)); // Redis에 저장
	...
	public SignInResponseDto oAuthSign(OAuthSignUpRequestDto oAuthSignUpRequestDto) {
		...
		OAuth oAuth = oAuthRepository.findByProviderAndProviderAccountId(oAuthSignUpRequestDto.getProvider(),
			oAuthSignUpRequestDto.getProviderAccountId()).orElse(null);
		...
		if (oAuth != null) {
			...
			redisTemplate.opsForValue()
				.set(refreshRedisToken.getRefreshToken(), oAuth.getUuid(), Duration.ofDays(1)); // Redis에 저장
		...
		userProfileService.createUserProfileService(createdUuid, socialNickname);
	...
	private Authentication authenticate(Auth auth, String inputPassword) {
		...
		return new UsernamePasswordAuthenticationToken(authUserDetail.getUsername(),  // 로그인 ID
			inputPassword);
	}

	private Authentication oAuthAuthenticate(String providerAccountId) {
		log.info("providerAccountId : {}", providerAccountId);
		return authenticationManager.authenticate(new OAuthAuthenticationToken(
			providerAccountId)); // AuthenticationManager가 OAuthAuthenticationToken을 토큰을 생성 후 검증하고 인증된 토큰을 반환
	}
	...
	public String generateRandomNickName() {
		String[] adjectives = {"happy", "funny", "famous", "smart", "strong", "shy", "sweet", "cool", "hot", "brave",
			"fast"};
