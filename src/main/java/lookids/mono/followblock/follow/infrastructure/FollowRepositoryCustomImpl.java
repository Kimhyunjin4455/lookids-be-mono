...
import static lookids.mono.followblock.follow.domain.QFollow.*;
...
public class FollowRepositoryCustomImpl implements FollowRepositoryCustom {
	...
	public Page<FollowResponseDto> findByFollowerUuid(String uuid, Pageable pageable) {
		List<Follow> followerList = jpaQueryFactory.selectFrom(follow)
		...
		// 총 데이터 개수 계산
		long total = jpaQueryFactory.select(follow.count()).from(follow).where(follow.followerUuid.eq(uuid)).fetchOne();

		// DTO로 변환
		List<FollowResponseDto> followResponseDtoList = followerList.stream().map(follow -> {
	...
	public Page<FollowResponseDto> findByFollowingUuid(String uuid, Pageable pageable) {
		List<Follow> followingList = jpaQueryFactory.selectFrom(follow)
		...
		// 총 데이터 개수 계산
		long total = jpaQueryFactory.select(follow.count())
			.from(follow)
			.where(follow.followingUuid.eq(uuid))
			.fetchOne();

		// DTO로 변환
		List<FollowResponseDto> followResponseDtoList = followingList.stream().map(follow -> {
			FollowResponseDto followResponseDto = new FollowResponseDto();
			followResponseDto.setFollowerUuid(follow.getFollowerUuid());
			followResponseDto.setFollowingUuid(follow.getFollowingUuid());
			return followResponseDto;
