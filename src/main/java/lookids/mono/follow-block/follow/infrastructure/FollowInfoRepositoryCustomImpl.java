...
import static Lookids.follow.domain.QFollowInfo.*;
...
public class FollowInfoRepositoryCustomImpl implements FollowInfoRepositoryCustom {
	...
	public Page<FollowInfoResponseDto> findByReceiverUuid(String uuid, Pageable pageable) {
		List<FollowInfo> followerList = jpaQueryFactory
			.selectFrom(followInfo)
		...
		long total = jpaQueryFactory
			.select(followInfo.count())
			.from(followInfo)
			.where(followInfo.receiverUuid.eq(uuid))
			.fetchOne();

		// DTO로 변환
		List<FollowInfoResponseDto> followerInfoList = followerList.stream()
			.map(followInfo -> {
	...
	public Page<FollowInfoResponseDto> findBySenderUuid(String uuid, Pageable pageable) {
		List<FollowInfo> followingList = jpaQueryFactory
			.selectFrom(followInfo)
		...
		// DTO로 변환
		List<FollowInfoResponseDto> followingInfoList = followingList.stream()
			.map(follow -> {
